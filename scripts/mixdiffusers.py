...
BBOX_MAX_NUM = min(shared.cmd_opts.md_max_regions if hasattr(shared.cmd_opts, "md_max_regions") else 6, 16)
                ...
                with gr.Accordion('Region Prompt Control', open=False):
                    ...
                    for i in range(BBOX_MAX_NUM):
                        with gr.Accordion(f'Region {i+1}', open=False):
                            ...
                            p = gr.Text(show_label=False, placeholder=f'Prompt, will be appended to your {tab} prompt)', max_lines=2, elem_id=f'MD-p-{i}')
                            neg = gr.Text(show_label=False, placeholder=f'Negative Prompt, will be appended too.', max_lines=1, elem_id=f'MD-p-{i}')
